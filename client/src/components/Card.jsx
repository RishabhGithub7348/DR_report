     ...
     >
        {
           ...
           (data.urlToImage && data.description.length>80 && data.title.length>10 )&&
            <Container textAlign={'center'}>
            <Image src={data.urlToImage}  alt="prod" />
            <Published>Pubished: {dt}-{mon}-{yr}</Published>
            <Heads >{(data.title).length>60&&(data.title).substring(0,60)}...</Heads>
            <Typography style={{textAlign:'justify'}}>{(data.description).length>100&&(data.description).substring(0,100)}...</Typography>
